...
class App:
	...
	def rebuildHostfile(self): 
		os.system('xterm -T Rebuilding\ Hostfile\ Blocklist... -e "bash -c \'hostfileblocklist;echo Program is now finished press enter to close the window...;read;\'"')
		#~ print 'lol'
	def editProtectedDomains(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/protectedDomains.source')
	def editExternalHosts(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/hostfiles.source')
	def editExternalDomains(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/domainLists.source')
	def editRedirects(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/redirectList.source')
	def editUnblockList(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/unblockList.source')
	def editSupplementalHostsFile(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/supplementalBlocklist.host')
	def editConfig(self):
		os.system('gksu leafpad /etc/hostfileBlocklist/config.source')
	def showAbout(self):
